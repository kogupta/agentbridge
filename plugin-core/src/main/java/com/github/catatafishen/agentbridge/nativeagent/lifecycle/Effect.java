 ...
 * or through a generic rethrow, records {@code FAILED_AFTER_START} and is rethrown as the same object. Expected
 * tool failures are values returned normally, so a throwable means an unexpected failure after the effect started.
 * The driver then records that call's result, starts no later call, makes no further provider request, and calls
 * {@link RunLifecycle#stop} so the remaining calls are cancelled before {@link RunLifecycle#finishRun}.
 *
