...
| Abort signal | `agent-loop.ts:677-718` passes `signal` into `tool.execute` and awaits the tool | Stop is delivered through the admission boundary. How a running effect observes Stop is open: `lifecycle-admission/review.md` round 3 M-002. |
