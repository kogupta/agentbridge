...
| `action step_mut_<X>`, `action step_mut_<X>__<variant>` | `quint verify --step=step_mut_<X>[__<variant>] --invariant=<X> --backend=tlc` | violation (the invariant detects the fault) |
...
3. **Put each mutant in the base module** as `mut_<name>` plus `action step_mut_<X> = any { step, mut_<name> }`. Put one mutant in each step. For a second mutant of the same invariant, name the step `step_mut_<X>__<variant>`. Do not write a mutant module that imports the base with `import ... as B`: Quint 0.32.0 cannot flatten those names for TLC or Apalache (`QNT404 Name 'B::...' not found`), so the check crashes.
